...
{
    HAL_GPIO_WritePin(POWER_PORT, POWER_PIN,
...
static void optical_flow_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (current_motion.min_raw_data == 0 &&
            current_motion.max_raw_data == 0) {
...
void optical_flow_module_update(state_t *state, float dt)
{
    ...
    if (is_sensor_ok && height_m < OPFLOW_MAX_HEIGHT_M) {
        ...
        vel_lpf[X] = constrainf(vel_lpf[X], -OPFLOW_VEL_LIMIT,
                                OPFLOW_VEL_LIMIT);
        vel_lpf[Y] = constrainf(vel_lpf[Y], -OPFLOW_VEL_LIMIT,
                                OPFLOW_VEL_LIMIT);
