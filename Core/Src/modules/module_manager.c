...
#define MODULE_DEBOUNCE_THRESHOLD \
  3 /**< consecutive matching reads before transition */
#define MODULE_DETECT_PERIOD_MS 500 /**< polling interval in milliseconds */
#define MODULE_ADC_TOLERANCE \
...
static void module_handler_led_ring_init(void)
{
  ledring_module_init();
}
static void module_handler_led_ring_deinit(void)
{
  ledring_module_deinit();
}
static void module_handler_wifi_init(void)
{
  wifi_module_init();
}
static void module_handler_wifi_deinit(void)
{
  wifi_module_deinit();
}
static void module_handler_optical_flow_init(void)
{
  optical_flow_module_init();
}
static void module_handler_optical_flow_deinit(void)
{
  optical_flow_module_deinit();
}
...
static const module_handler_entry_t handler_table[] = {
  ...
  { BSP_MODULE_LED_RING, module_handler_led_ring_init,
    module_handler_led_ring_deinit },
  { BSP_MODULE_WIFI_CAMERA, module_handler_wifi_init,
    module_handler_wifi_deinit },
  { BSP_MODULE_OPTICAL_FLOW, module_handler_optical_flow_init,
    module_handler_optical_flow_deinit },
  { BSP_MODULE_RESERVED_1, module_handler_reserved_init,
    module_handler_reserved_deinit },
};
...
{
  for (uint32_t i = 0; i < sizeof(handler_table) / sizeof(handler_table[0]);
       i++) {
 ...
 */
static void module_apply_transition(bsp_module_id_t old_id,
                                    bsp_module_id_t new_id)
{
