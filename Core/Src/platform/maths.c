...
float maths_sin(float x)
{
  ...
  return x +
         x * x2 *
           (SIN_COEF3 + x2 * (SIN_COEF5 + x2 * (SIN_COEF7 + x2 * SIN_COEF9)));
}
...
#define ATAN_COEF7 0.6444640676891548f
  ...
  res = -((((ATAN_COEF5 * res - ATAN_COEF4) * res - ATAN_COEF3) * res -
           ATAN_COEF2) *
            res -
          ATAN_COEF1) /
        ((ATAN_COEF7 * res + ATAN_COEF6) * res + 1.0f);
...
void maths_stdev_push(maths_stdev_t *dev, float x)
{
  ...
  if (dev->m_n == 1) {
    ...
    dev->m_new_s = dev->m_old_s + (x - dev->m_old_m) * (x - dev->m_new_m);
...
float maths_scale_f(float x, float src_min, float src_max, float dst_min,
                    float dst_max)
{
  return (dst_max - dst_min) * (x - src_min) / (src_max - src_min) + dst_min;
}
...
void maths_vec3_rotate(Axis3f *v, const EulerAngle *delta)
{
  ...
  v->x = tmp.x * m[0][AXIS_X] + tmp.y * m[1][AXIS_X] + tmp.z * m[2][AXIS_X];
  v->y = tmp.x * m[0][AXIS_Y] + tmp.y * m[1][AXIS_Y] + tmp.z * m[2][AXIS_Y];
  v->z = tmp.x * m[0][AXIS_Z] + tmp.y * m[1][AXIS_Z] + tmp.z * m[2][AXIS_Z];
}
