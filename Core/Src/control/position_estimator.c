                               ...
                               const pos_estimator_ext_t *ext, float dt)
{
  ...
  if (has_opflow && ext->laser_quality >= LASER_QUALITY_MIN) {
    pes.range_lpf = lpf_step(pes.range_lpf, ext->laser_range, RANGE_LPF_ALPHA);

    float quality = ext->laser_quality;
    weight = quality;
    pes.baro_ground_asl = sensor->baro.asl - pes.range_lpf;
    pes.fused_height = pes.range_lpf * quality + (1.0f - quality) * rel_height;
  } else {
    pes.fused_height = rel_height;
  }

  pes.fused_height_lpf =
    lpf_step(pes.fused_height_lpf, pes.fused_height, FUSED_H_LPF_ALPHA);
  ...
  for (int i = 0; i < 3; i++)
    pes.acc_lpf[i] = lpf_step(pes.acc_lpf[i], pes.acc_earth[i], ACC_LPF_ALPHA);
