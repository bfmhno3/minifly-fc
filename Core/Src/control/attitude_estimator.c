...
void attitude_estimator_update(const Axis3f *acc, const Axis3f *gyro, float dt)
{
  ...
  {
    float qnorm = 1.0f / sqrtf(est.q0 * est.q0 + est.q1 * est.q1 +
  ...
  if (!est.calibrated) {
    float earth_z = acc->x * est.r_mat[2][0] + acc->y * est.r_mat[2][1] +
                    acc->z * est.r_mat[2][2];
...
{
  float x =
    est.r_mat[0][0] * v->x + est.r_mat[0][1] * v->y + est.r_mat[0][2] * v->z;
  float y =
    est.r_mat[1][0] * v->x + est.r_mat[1][1] * v->y + est.r_mat[1][2] * v->z;
  float z =
    est.r_mat[2][0] * v->x + est.r_mat[2][1] * v->y + est.r_mat[2][2] * v->z;
...
{
  float x =
    est.r_mat[0][0] * v->x + est.r_mat[1][0] * v->y + est.r_mat[2][0] * v->z;
  float y =
    est.r_mat[0][1] * v->x + est.r_mat[1][1] * v->y + est.r_mat[2][1] * v->z;
  float z =
    est.r_mat[0][2] * v->x + est.r_mat[1][2] * v->y + est.r_mat[2][2] * v->z;
