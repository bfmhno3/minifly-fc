		      ...
		      attitude_t *att_out, float *thrust_out)
{
	...
	if (sp->mode.x == MODE_ABS)
		sp->velocity.x = POS_XY_SCALE *
			pid_update(&pid_x, sp->position.x - state->position.x);

	if (sp->mode.y == MODE_ABS)
		sp->velocity.y = POS_XY_SCALE *
			pid_update(&pid_y, sp->position.y - state->position.y);

	if (sp->mode.z == MODE_ABS)
		sp->velocity.z = pid_update(&pid_z,
			sp->position.z - state->position.z);

	/* inner loop: velocity -> attitude / thrust */
	att_out->pitch = VEL_XY_SCALE *
		pid_update(&pid_vx, sp->velocity.x - state->velocity.x);
	att_out->roll = VEL_XY_SCALE *
		pid_update(&pid_vy, sp->velocity.y - state->velocity.y);

	float thrust_raw = pid_update(&pid_vz,
		sp->velocity.z - state->velocity.z);
