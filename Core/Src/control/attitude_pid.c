...
void attitude_pid_run_angle(const attitude_t *actual,
                            const attitude_t *desired,
                            Axis3f *desired_rate)
{
	float yaw_error;

	desired_rate->x = pid_update(&pid_angle_roll,
...
void attitude_pid_run_rate(const Axis3f *actual_rate,
                           const Axis3f *desired_rate,
                           Axis3f *control_out)
{
	control_out->x = pid_update(&pid_rate_roll,
	                            desired_rate->x - actual_rate->x);
	control_out->y = pid_update(&pid_rate_pitch,
	                            desired_rate->y - actual_rate->y);
	control_out->z = pid_update(&pid_rate_yaw,
	                            desired_rate->z - actual_rate->z);
}
